...
public class ConditionalInferenceStrategy extends AbstractInferenceStrategy {
	...
	private ITypeBinding inferThenElse(Expression thenExpression,
		...
		if (thenSafe && !elseSafe) {
			...
			// This is just in case safety of thenExpression was wrong...
			// It happens for expressions not yet covered by an inference strategy.
			if (finalBinding == null) {
				finalBinding = elseExpression.resolveTypeBinding();
			}
		}
		return finalBinding;
	}
