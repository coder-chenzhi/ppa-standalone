...
/**
 * <p>
 * ASTVisitor that retrieves a map of all names in a compilation unit along with
 * their binding and whether the name is a declaration (def vs use).
 * </p>
 * 
 * <p>
 * Once the ASTVisitor has run, users can retrieve four properties for each
 * name: the binding (string format), the kind of the name (e.g., type, field,
 * method), if the name was a declaration or a use, and the ASTNode. The four
 * lists have the same length and each name has the same index in the four list.
 * </p>
 * 
...
public class NameMapVisitor extends ASTVisitor {
	 ...
	 * 
	 * @param encoded
	 *            if True, the string representing the binding will be in the
	 *            form of a SemDiff handle.
	 * @param augmented
	 *            if True, the binding will be augmented with additional type
	 *            information (e.g., if a type is an annotation).
	 * @param filterUnknown
	 *            if True, unknown members and snippet members introduced by PPA
	 *            are filtered out.
	 */
	public NameMapVisitor(boolean encoded, boolean augmented,
...
//			IMemberValuePairBinding mvpBinding = mvp
//					.resolveMemberValuePairBinding();
//			IBinding binding = null;
//			if (mvpBinding != null) {
//				binding = mvpBinding.getMethodBinding();
//			}
//			if (binding != null) {
//				addBindingText(binding, mvp.getName(), false);
//			}
		}
	}

	private String addBindingText(IBinding binding, ASTNode node,
	...
	/**
	 * 
	 * @return A list of bindings in string format. A binding can be in the form
	 *         of a SemDiff handle or in human-readable form.
	 */
	...
	/**
	 * 
	 * @return A list of binding types (e.g., a type, a field, a method). This
	...
	/**
	 * 
	 * @return A list of booleans indicating whether an expression was a
	 *         declaration (True) or a usage (False).
	 */
