...
public class PPABindingsUtil {
	...
	public static void fixConstructor(ClassInstanceCreation cic, PPATypeRegistry typeRegistry,
		...
		ITypeBinding[] paramTypes = PPABindingsUtil.getParamTypes(cic.arguments(), unknownType);
		...
		if (acceptableMethods.size() == 1) {
			...
			if (oldBinding == null || !newBinding.isEqualTo(oldBinding)) {
				...
				reportNewConstructorBinding(indexer, engine, unknownType, cic.arguments(), paramTypes,
						newBinding);
			}
		} else {
	...
	public static void fixMethod(SimpleName name, ITypeBinding expectedReturnType,
			...
			PPAEngine ppaEngine) {
		fixMethod(name, expectedReturnType, typeRegistry, resolver, indexer, ppaEngine, true, false);
	}
			...
			if (!skipSuperMissing
				...
				if (expectedReturnType == null) {
					acceptableMethods = PPABindingsUtil.filterMethods(PPABindingsUtil
							.findAcceptableMethods(methodName, paramTypes, containerType),
							filterUnknowns);
				} else {
					acceptableMethods = PPABindingsUtil.filterMethods(PPABindingsUtil
	...
	public static ASTNode getCU(ICompilationUnit icu) throws JavaModelException {
	...
	public static ASTNode getCU(ICompilationUnit icu, boolean cleanup) throws JavaModelException {
		PPATypeRegistry registry = new PPATypeRegistry((JavaProject) JavaCore.create(icu
				.getUnderlyingResource().getProject()));
	...
	// For testing
	public static ASTNode getCU(ICompilationUnit icu, PPATypeRegistry registry) throws JavaModelException {
		return getCU(icu, registry, true);
	}
	...
	public static ASTNode getCUWithoutMethodBinding(ICompilationUnit icu) throws JavaModelException {
		PPATypeRegistry registry = new PPATypeRegistry((JavaProject) JavaCore.create(icu
				.getUnderlyingResource().getProject()));
		ASTNode node = null;
		PPAASTParser parser2 = new PPAASTParser(AST.JLS3);
	...
	public static ITypeBinding[] getParamTypes(List args, ITypeBinding unknownBinding) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (tempBinding == null || tempBinding.isNullType()) {
	...
	private static void reportNewConstructorBinding(PPAIndexer indexer, PPAEngine engine,
			ITypeBinding unknownType, List args, ITypeBinding[] paramTypes,
...
//			compilationUnitDeclaration.resolve();
	...
	public static void fixThisConstructor(ConstructorInvocation ci, PPATypeRegistry typeRegistry,
			PPADefaultBindingResolver resolver, PPAIndexer indexer, PPAEngine engine,
			boolean skipSuperMissing, boolean filterUnknowns) {
		IMethodBinding oldBinding = ci.resolveConstructorBinding();
		ITypeBinding unknownType = typeRegistry.getUnknownBinding(resolver);
		ITypeBinding[] paramTypes = PPABindingsUtil.getParamTypes(ci.arguments(), unknownType);
		ITypeBinding containerType = null;
		...
		// not happen.
		if (containerType == null) {
			logger.info("This container type was null: " + ci.toString());
			containerType = unknownType;
		}
		...
		if (acceptableMethods.size() == 1) {
			IMethodBinding newBinding = acceptableMethods.get(0);
			if (oldBinding == null || !newBinding.isEqualTo(oldBinding)) {
				resolver.fixConstructorInvocation(ci, newBinding);
				reportNewConstructorBinding(indexer, engine, unknownType, ci.arguments(), paramTypes,
						newBinding);
			}
		} else {
			IMethodBinding newBinding = typeRegistry.createConstructor(containerType, paramTypes,
