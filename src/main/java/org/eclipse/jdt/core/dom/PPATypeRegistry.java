...
public class PPATypeRegistry {
	...
	public IMethodBinding createMethodBinding(String name,
		...
		if (tempContainerType.isBaseType()) {
			...
			internalContainerType = getReferenceBinding(PPABindingsUtil
					.getInternalTypeBinding(container), resolver);
		}
	...
	public MethodBinding createInternalMethodBinding(String name,
			ITypeBinding container, ITypeBinding returnType,
			ITypeBinding[] paramBindings, BindingResolver resolver) {
		MethodBinding mBinding = null;
		int size = paramBindings.length;
	...
	private ITypeBinding createTypeBinding(String name, String fqn,
	...
	public ReferenceBinding getReferenceBinding(
		...
		if (typeBinding instanceof ReferenceBinding) {
			reference = (ReferenceBinding) typeBinding;
		} else if (typeBinding instanceof ArrayBinding) {
			typeBinding = ((ArrayBinding) typeBinding).leafComponentType();
			if (typeBinding instanceof ReferenceBinding) {
				reference = (ReferenceBinding) typeBinding;
			} else {
				reference = getInternalUnknownBinding(resolver);
			}
		} 
